...
function clientContext(req, url) {
  ...
  const storedAttr = decodeAttribution(cookies.hd_attr);
  const attr = Object.keys(incomingAttr).length ? { ...storedAttr, ...incomingAttr } : storedAttr;
  ...
  const setCookies = [
    ...
    cookie('hd_attr', encodeAttribution(attr), { secure: config.cookieSecure, maxAge: 60 * 60 * 24 * 30 }),
  ];

  return { visitorId, sessionId, attr, setCookies };
}
...
function toEvent(req, url, ctx, eventName, extra = {}) {
  return {
    ...
    projectId: akimitsu.id,
    visitorId: ctx.visitorId,
    sessionId: ctx.sessionId,
    eventName,
    destinationKey: extra.destinationKey,
    destinationProvider: extra.destinationProvider,
    source: ctx.attr.utm_source,
    medium: ctx.attr.utm_medium,
    campaignId: ctx.attr.utm_id || ctx.attr.campaign_id,
    campaignName: ctx.attr.utm_campaign,
    content: ctx.attr.utm_content,
    term: ctx.attr.utm_term,
    gclid: ctx.attr.gclid,
    fbclid: ctx.attr.fbclid,
    referrer: ctx.attr.referrer || req.headers.referer || '',
    path: url.pathname,
...
async function handle(req, res) {
  ...
  if (url.pathname.startsWith('/api/performance')) {
    ...
    return json(res, 200, {
      project: akimitsu.id,
      days,
      ...(await eventSummary(akimitsu.id, since)),
    });
  }
  ...
  if (url.pathname === '/' || url.pathname === '/menu') {
    const ctx = clientContext(req, url);
    if (!isLikelyBot(req.headers['user-agent'] || '')) {
      await recordEvent(toEvent(req, url, ctx, 'page_view'));
    }
...
server.listen(config.port, '0.0.0.0', () => {
  console.log(`hangdoi-measurement listening on :${config.port} (${dbMode()})`);
});
